...
final class Sign_Docs_Document_Service
{
    ...
    public static function create_from_local_pdf(string $source_path, array $args)
    {
        ...
        $meta = array(
            ...
            'stamp_padding' => isset($args['stamp_padding']) ? (string) round(min(16, max(2, (float) $args['stamp_padding'])), 1) : '5',
            'stamp_qr_gap' => isset($args['stamp_qr_gap']) ? (string) round(min(20, max(0, (float) $args['stamp_qr_gap'])), 1) : '5',
            'stamp_qr_padding' => isset($args['stamp_qr_padding']) ? (string) round(min(12, max(0, (float) $args['stamp_qr_padding'])), 1) : '5',
            'stamp_qr_size' => isset($args['stamp_qr_size']) ? (string) round(min(120, max(20, (float) $args['stamp_qr_size'])), 1) : '54',
            'stamp_qr_ec_level' => isset($args['stamp_qr_ec_level']) && in_array(sanitize_key((string) $args['stamp_qr_ec_level']), array('l', 'm', 'q', 'h'), true) ? sanitize_key((string) $args['stamp_qr_ec_level']) : 'h',
            'stamp_page' => isset($args['stamp_page']) && in_array(sanitize_key((string) $args['stamp_page']), array('last', 'both'), true) ? sanitize_key((string) $args['stamp_page']) : 'first',
            'stamp_footer_enabled' => array_key_exists('stamp_footer_enabled', $args) ? (! empty($args['stamp_footer_enabled']) && '0' !== (string) $args['stamp_footer_enabled'] ? '1' : '0') : '1',
            ...
            'stamp_qr_position' => isset($args['stamp_qr_position']) && 'below' === sanitize_key((string) $args['stamp_qr_position']) ? 'below' : 'right',
